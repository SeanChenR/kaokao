...
- [x] 3.1 e2e:全答對(用已知答案作答)→ 結果頁分數 5/5+高亮+confetti canvas 存在;部分作答 → 對應文案;結果頁 reload 分數/高亮不變且不重複寫榜;再玩一次 → 開始頁預覽含新紀錄(spec 各項);驗證:playwright 綠
- [x] 3.2 README 註明 demo seed;全套 gate + 結果頁雙主題×雙視口截圖(滿分/部分兩種)。驗證:指令零錯+截圖目視
