...
3. **破音字策略**:題庫內容必含至少 3 組「同字兩讀」對照(如 長:ㄔㄤˊ 長度/ㄓㄤˇ 長大;行:ㄒㄧㄥˊ 行走/ㄏㄤˊ 銀行;樂:ㄌㄜˋ 快樂/ㄩㄝˋ 音樂),分佈於不同題(長/行/樂為詞義破音字;「一」的四聲前變調另作補充組,不佔破音字配額)。`src/data/polyphone.test.ts` 為 golden 測試:硬編這些字在特定題的正確讀音,題庫改動若標錯即紅燈。
