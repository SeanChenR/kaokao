...
import { answerCorrectly, currentQuestion, nextQuestion, zt } from "./helpers";
...
test("play again returns to start with new record in preview", async ({ page }) => {
  ...
  await expect(page.getByText(zt("今夜排行榜")).first()).toBeVisible();
  await expect(page.getByText("預覽星").first()).toBeVisible(); // 滿分必在前五
});
